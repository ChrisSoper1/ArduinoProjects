 ...
 */
int SENSOR_LIST[NUM_OF_SENSORS]  = { SENS_1, SENS_2, SENS_3, SENS_4, SENS_5, SENS_6  };
int LED_LIST[NUM_OF_SENSORS]     = {  LED_1,  LED_2,  LED_3,  LED_4,  LED_5,  LED_6  };
int *VALUE_LIST[NUM_OF_SENSORS];
//   = {   val1,   val2,   val3,   val4,   val5,   val6  };

void setup() {
  VALUE_LIST[0] = &val1; VALUE_LIST[1] = &val2; VALUE_LIST[2] = &val3;
  VALUE_LIST[3] = &val4; VALUE_LIST[4] = &val5; VALUE_LIST[5] = &val6;
...
void loop() {
  ...
  if ( digitalRead(GO_LEVER) == LOW && strike_count <3) // Three tries
  {
     ...
     *  
     *  We also use this opprotunity to turn off all LEDs to start
     */
    ...
    for ( int blink_counter = 0; blink_counter < 7; blink_counter = blink_counter+1 )
    {
     ...
     for (int led_idx = 0; led_idx < NUM_OF_SENSORS; led_idx++)
     {
      ...
      {
        // loop over all unchecked leds (start at current led_index + 1) and set to random state
        for (int unchked_idx = (led_idx + 1); unchked_idx < NUM_OF_SENSORS; unchked_idx++)
        {
          ...
          //digitalWrite( LED_LIST[unchked_idx], random(0,2));
        }
        delay (300);
      }      
    }

    if (!val1 && !val2 && !val3 && !val4 && !val5 && !val6) //all answers correct
    {
